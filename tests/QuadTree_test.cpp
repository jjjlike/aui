...
protected:
    void SetUp() override {
        ...
        tree = std::make_unique<QuadTree>(Rect{0, 0, 1000, 1000});
    }
    
    std::unique_ptr<QuadTree> tree;  // 四叉树指针
};
...
TEST_F(QuadTreeTest, EmptyQuery) {
    ...
    auto result = tree->query(Point{500, 500});
    // 验证查询结果为空
...
TEST_F(QuadTreeTest, SingleComponent) {
    // 创建三个组件句柄
    ComponentHandle h1{0, 1};
    ComponentHandle h2{1, 1};
    ComponentHandle h3{2, 1};
    
    // 将组件添加到向量中
    std::vector<ComponentHandle> components = {h1, h2, h3};
    // 设置每个组件的边界框（位置和大小）
    ...
    auto result = tree->query(Point{30, 30});
    // 验证结果不为空
...
TEST_F(QuadTreeTest, HitTestMiss) {
    // 创建一个组件句柄
    ComponentHandle h1{0, 1};
    std::vector<ComponentHandle> components = {h1};
    // 设置组件的边界框为 (10, 10) 到 (60, 60) 的区域
    std::vector<Rect> bounds = {{10, 10, 50, 50}};
...
TEST_F(QuadTreeTest, MultipleHits) {
    ...
    std::vector<ComponentHandle> components = {h1, h2};
    ...
    auto result = tree->query(Point{75, 75});
    // 验证两个组件都被命中
...
TEST_F(QuadTreeTest, RectQuery) {
    ...
    auto result = tree->query(Rect{0, 0, 100, 100});
    // 验证命中了 1 个组件
...
TEST_F(QuadTreeTest, ManyComponents) {
    ...
    auto result = tree->query(Point{50, 50});
    // 验证结果数量大于 0
...
TEST_F(QuadTreeTest, UpdateComponent) {
    ...
    // 查询原来的位置 (30, 30)，应该未命中
    auto missResult = tree->query(Point{30, 30});
    EXPECT_TRUE(missResult.empty());
    
    // 查询新的位置 (520, 520)，应该命中
    auto hitResult = tree->query(Point{520, 520});
