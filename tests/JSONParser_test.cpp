 ...
 *
 * 功能说明：
 * - 测试 JJSONParser 类的解析功能
 * - 测试 JJSONValue 类的存储和访问
 * - 测试 JSON 的序列化功能
...
TEST_F(JSONParserTest, ParseSimpleObject) {
    ...
    // 解析 JSON
    auto node = JJSONParser::parse(json);

    // 验证解析成功
    ASSERT_TRUE(std::holds_alternative<JJSONObject>(node.value));

    // 获取对象
    const auto& obj = std::get<JJSONObject>(node.value);
...
TEST_F(JSONParserTest, ParseArray) {
    ...
    // 验证解析成功
    ASSERT_TRUE(std::holds_alternative<JJSONArray>(node.value));

    // 获取数组
    const auto& arr = std::get<JJSONArray>(node.value);
...
/**
 * 测试用例 6：序列化对象（正常逻辑测试）
 * 测试目标：验证能正确将 JJSONValue 对象序列化为字符串
 */
TEST_F(JSONParserTest, StringifyObject) {
    // 创建一个 JSON 对象，包含名称和数值
    JJSONObject obj;
    obj["name"] = JJSONValue(std::string("Test"));
    obj["value"] = JJSONValue(42);

    // 序列化为字符串
    std::string json = JJSONParser::stringify(JJSONValue(std::move(obj)));
...
TEST_F(JSONParserTest, NestedObjects) {
    ...
    ASSERT_TRUE(std::holds_alternative<JJSONObject>(outerVal.value));
    const auto& outer = std::get<JJSONObject>(outerVal.value);

    // 访问内层对象
    const auto& innerVal = outer.at("inner");
    ASSERT_TRUE(std::holds_alternative<JJSONObject>(innerVal.value));
    const auto& inner = std::get<JJSONObject>(innerVal.value);
...
TEST_F(JSONParserTest, ArrayWithObjects) {
    ...
    // 验证第一个对象的结构
    ASSERT_TRUE(std::holds_alternative<JJSONObject>(arr[0].value));
    const auto& obj0 = std::get<JJSONObject>(arr[0].value);
