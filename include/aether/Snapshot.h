 ...
 */
struct Snapshot {
    std::vector<SnapshotComponent> components;  // 所有组件信息
...
public:
     ...
     */
    static void apply(const Snapshot& snapshot, ComponentStorage& storage);
     ...
     * @return 二进制数据
     */
    static std::vector<uint8_t> toBinary(const Snapshot& snapshot);
     ...
     * @return 快照对象
     */
    static Snapshot fromBinary(const std::vector<uint8_t>& data);
     ...
     * @return 如果相同返回true
     */
    static bool compare(const Snapshot& a, const Snapshot& b);
